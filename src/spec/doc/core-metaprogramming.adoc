...
Compile-time metaprogramming in Groovy allows code generation at compile-time. Those transformations alter the
Abstract Syntax Tree (AST) of a program, which is why in Groovy we call them AST transformations. AST transformations
...
and what the disadvantages of this technique are.
...
Groovy doesn't ship with any global AST transformations, but you can find a list of local AST transformations
available for you to use in your code here:

==== Code generation transformations

This category of transformations includes AST transformations which help in removing boilerplate code. This is typically
...
following code will generate three constructors:
...
e.g. a no-argconstructor and setters for the properties. Here is an example using a Java class:
...
are in a JAR added to the classpath of the compiler and contain a service locator file
`META-INF/services/org.codehaus.groovy.transform.ASTTransformation` with a line containaing the name of the
...
It will be run against *every source in the compilation*, so be sure not to create transformations which
...
past that. The element type here for `@WithLogging` is `METHOD` because the annotation
applies to methods.
...
<5> the `nodes` parameter is a two AST node array, for which the first one is the annotation node (`@WithLogging`) and
...
<11> create an `ExpressionStatement` wrapping a `MethodCallExpression` corresponding to `this.println("message")`
...
is going to be compiled in phases and each compile phase processes all files of the same source unit before going
...
Global AST transformations are similar to local ones with a major difference: they do not need an annotation, meaning that
they are applied _globally_, that is to say on each class being compiled. It is therefore very important to limit their
usage as a last resort, because it can have a significant impact on the compiler performance.
...
The descriptor file is required and must be found on the classpath. It will contain a single line:
...
<12> create an `ExpressionStatement` wrapping a `MethodCallExpression` corresponding to `this.println("message")`
